...
// NOTE: ScanTailor's gray-morphology polarity is inverted relative to the
// conventional naming used by the shaders: dilateGray spreads dark ink (a MIN
// filter), erodeGray eats it (a MAX filter). So metalDilateGray dispatches the
// min-filter ("erode") shaders and metalErodeGray the max-filter ("dilate")
// shaders.
