 ...
 */
static bool brickIsCenteredOdd(const Brick& brick) {
  return brick.minX() == -brick.maxX() && brick.minY() == -brick.maxY();
}

/**
 * vImage-accelerated grayscale dilation (ScanTailor semantics: a MIN filter —
 * gray dilation spreads dark ink). Apple's naming is inverted relative to ours:
 * vImageErode_Planar8 is the sliding-window minimum, so that is what we call.
 * The all-zero kernel is a flat structuring element (kernel values are
 * intensity offsets in vImage, not a mask). Returns null GrayImage on failure.
 */
static GrayImage vImageDilateGray(const GrayImage& src, const Brick& brick, unsigned char /*borderValue*/) {
  ...
  if (brickWidth < 1 || brickHeight < 1 || !brickIsCenteredOdd(brick)) {
    return GrayImage();
  }

  // Flat structuring element
  std::vector<uint8_t> kernel(brickWidth * brickHeight, 0);
...
/**
 * vImage-accelerated grayscale erosion (ScanTailor semantics: a MAX filter —
 * gray erosion eats dark ink). Apple's vImageDilate_Planar8 is the
 * sliding-window maximum. Returns null GrayImage on failure.
 */
...
#ifdef Q_OS_MACOS
  // Accelerated paths handle only the simple case: full-rect destination and a
