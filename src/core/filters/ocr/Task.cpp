  ...
  if (m_settings->ocrEnabled()) {
    ...
    if (!m_settings->hasOcrResult(m_pageId)) {
      if (!outputImage.isNull()) {
        qDebug() << "OCR: Running OCR on in-memory output image"
                 << "size:" << outputImage.width() << "x" << outputImage.height();
        performOcr(outputImage);
      } else if (QFile::exists(outputFilePath)) {
        outputImage.load(outputFilePath);
        if (!outputImage.isNull()) {
          qDebug() << "OCR: Running OCR on fallback output image" << outputFilePath
                   << "size:" << outputImage.width() << "x" << outputImage.height();
          performOcr(outputImage);
        } else {
      ...
      if (cached && cached->imageWidth() > 0 && cached->imageHeight() > 0) {
        ...
        if (!outputImage.isNull()) {
          qDebug() << "OCR: Re-running OCR on in-memory output image";
          performOcr(outputImage);
        } else if (QFile::exists(outputFilePath)) {
          outputImage.load(outputFilePath);
          if (!outputImage.isNull()) {
            qDebug() << "OCR: Re-running OCR on fallback output image" << outputFilePath;
            performOcr(outputImage);
          }
        }
      }
