...
QStringList supportedLanguageCodes() {
  // Empty preserves Vision's fast system-language default. True multilingual
  // detection is explicit because it is substantially slower on long books.
  return QStringList() << ""  // System language
                       << "auto"
                       << "en-US"
                       << "fr-FR"
                       << "de-DE"
...
QString languageDisplayName(const QString& code) {
  if (code.isEmpty()) {
    return QCoreApplication::translate("ocr::Settings", "System language (fast)");
  }
  if (code == "auto") {
    return QCoreApplication::translate("ocr::Settings", "Auto-detect (slower)");
  }
  if (code == "en-US") {
...
void Settings::setUseAccurateRecognition(bool accurate) {
  QMutexLocker locker(&m_mutex);
  if (m_useAccurateRecognition == accurate) {
    return;
  }
...
bool Settings::useLanguageCorrection() const {
  QMutexLocker locker(&m_mutex);
  return m_useLanguageCorrection;
}

void Settings::setUseLanguageCorrection(bool enabled) {
  QMutexLocker locker(&m_mutex);
  if (m_useLanguageCorrection == enabled) {
    return;
  }
  m_useLanguageCorrection = enabled;
  m_perPageResults.clear();
}
