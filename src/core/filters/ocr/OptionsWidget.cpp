...
OptionsWidget::OptionsWidget(std::shared_ptr<Settings> settings, const PageSelectionAccessor& pageSelectionAccessor)
  ...
  connect(fastOcrCB, &QCheckBox::toggled, this, &OptionsWidget::fastOcrChanged);
  connect(multilingualCB, &QCheckBox::toggled, this, &OptionsWidget::multilingualChanged);
  connect(skipLanguageCorrectionCB, &QCheckBox::toggled,
          this, &OptionsWidget::skipLanguageCorrectionChanged);
...
void OptionsWidget::populateLanguageCombo() {
  ...
  for (const QString& code : codes) {
    // Multilingual detection has its own visible checkbox.
...
void OptionsWidget::updateDisplay() {
  ...
  const QSignalBlocker blocker3(fastOcrCB);
  const QSignalBlocker blocker4(multilingualCB);
  const QSignalBlocker blocker5(skipLanguageCorrectionCB);

  const bool ocrEnabled = m_settings->ocrEnabled();
  const QString currentLang = m_settings->language();
  const bool multilingual = currentLang == QStringLiteral("auto");
  enableOcrCB->setChecked(ocrEnabled);
  fastOcrCB->setChecked(!m_settings->useAccurateRecognition());
  multilingualCB->setChecked(multilingual);
  skipLanguageCorrectionCB->setChecked(!m_settings->useLanguageCorrection());

  // Enable/disable controls based on OCR enabled state
  languageLabel->setEnabled(ocrEnabled && !multilingual);
  languageCombo->setEnabled(ocrEnabled && !multilingual);
  fastOcrCB->setEnabled(ocrEnabled);
  multilingualCB->setEnabled(ocrEnabled);
  skipLanguageCorrectionCB->setEnabled(ocrEnabled);
  resultsGroup->setEnabled(ocrEnabled);

  // Keep the explicit choice while multilingual detection temporarily owns
  // the Settings language value.
  if (!multilingual) {
    m_languageBeforeMultilingual = currentLang;
...
void OptionsWidget::languageChanged(int index) {
  if (index >= 0) {
    ...
    m_languageBeforeMultilingual = langCode;
    m_settings->setLanguage(langCode);
  }
}

void OptionsWidget::fastOcrChanged(bool checked) {
  m_settings->setUseAccurateRecognition(!checked);
}

void OptionsWidget::multilingualChanged(bool checked) {
  if (checked) {
    const QString language = m_settings->language();
    if (language != QStringLiteral("auto")) {
      m_languageBeforeMultilingual = language;
    }
    m_settings->setLanguage(QStringLiteral("auto"));
  } else {
    m_settings->setLanguage(m_languageBeforeMultilingual);
  }
  updateDisplay();
}

void OptionsWidget::skipLanguageCorrectionChanged(bool checked) {
  m_settings->setUseLanguageCorrection(!checked);
}
