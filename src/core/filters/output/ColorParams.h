...
namespace output {
// AUTO_DETECT is a transient UI request and is never stored. Finalize may
// conservatively select Mixed when its inexpensive analysis finds a localized
// continuous-tone region on an otherwise document-like page.
// COLOR outputs RGB, while GRAYSCALE outputs 8-bit grayscale regardless of input.
// COLOR_GRAYSCALE is retained for backward compatibility and behaves like COLOR.
...
class ColorParams {
 public:
  ...
  bool isColorModePresetSet() const;

  void setColorModePresetSet(bool presetSet);
 ...
 private:
  ...
  bool m_colorModePresetSet;  // true if Force B&W Auto Process supplied it
