...
QLineF SpineDarknessFinder::findSpine(const GrayImage& grayDownscaled,
  ...
  if (preferAnchorIfBroadGutter) {
    ...
    if (foundBroadGutter) {
      ...
      // case for real-book broad-gutter failures; keep rightward moves tiny.
  ...
  auto evaluateColumn = [&](double xT, double xB, double meanDark, int darkRows, int sampledRows,
    ...
    if (outDrf < kMinDarkRowFraction) {
      // Faint binding fold against a text/photo edge. A real-book test case
      // has the visible gutter at only ~51% dark rows, so it misses the
      // normal 0.55 continuity gate. The local
