...
namespace {
// QStorageInfo needs an existing path, so climb to the nearest existing ancestor.
QStorageInfo volumeOf(const QString& path) {
  ...
  return QStorageInfo(existing);
}

// Paths on the project file's volume are stored relative to the project file,
// so the project folder can move between machines. Cross-volume paths stay
// absolute — a relative path through a mount point would break as soon as
// either side moves independently.
QString toRelativeIfSameVolume(const QString& projectDir, const QString& absolutePath) {
  if (absolutePath.isEmpty() || !QDir::isAbsolutePath(absolutePath)) {
    return absolutePath;
  }
  const QStorageInfo projectVolume = volumeOf(projectDir);
  const QStorageInfo pathVolume = volumeOf(absolutePath);
  if (!projectVolume.isValid() || !pathVolume.isValid() || (projectVolume.rootPath() != pathVolume.rootPath())) {
    return absolutePath;
  }
  const QString relative = QDir(projectDir).relativeFilePath(absolutePath);
...
bool ProjectWriter::write(const QString& filePath, const std::vector<FilterPtr>& filters) const {
  ...
  rootEl.setAttribute("outputDirectory", toRelativeIfSameVolume(projectDir, m_outFileNameGen.outDir()));
...
QDomElement ProjectWriter::processDirectories(QDomDocument& doc, const QString& projectDir) const {
  ...
  for (const Directory& dir : m_dirs.get<Sequenced>()) {
    ...
    dirEl.setAttribute("path", toRelativeIfSameVolume(projectDir, dir.path));
