 ...
 */
int countHighMidtoneRegions(PIX* gray, int midtoneThreshold) {
  ...
  // A higher user threshold means greater sensitivity, matching the historical
  // control direction while retaining the census-derived 50% default.
 ...
 * @param midtoneThreshold Sensitivity adjustment for localized picture evidence
 */
GrayscaleToneType classifyGrayscaleTone(PIX* pix, float* midtoneRatio, int midtoneThreshold = 10) {
  ...
  if (!gray) return GrayscaleToneType::ContinuousTone;
  ...
  if (!histo) {
    pixDestroy(&gray);
    return GrayscaleToneType::ContinuousTone;
  }
  ...
  // Before declaring B&W, check for embedded images using the normalized image
  // already built above. A minimal page-wide gate rejects tiny marks and scan
  // noise before the sampled regional check. If nearly the entire interior is
  // continuous-tone, this is a full-page photograph rather than Mixed.
  if (lowMidtones && (bimodal || extremelyLight)) {
    ...
    if (midRatio >= 0.01f) {
      fprintf(stderr, "  Candidate B&W (%.1f%% midtones), checking regions for embedded images...\n",
              midRatio * 100);
      const int highMidtoneCells = countHighMidtoneRegions(gray, midtoneThreshold);
      if (highMidtoneCells > 0 && highMidtoneCells < 13) {
        fprintf(stderr, "  -> Localized continuous-tone region detected, treating as mixed\n");
        pixDestroy(&gray);
        return GrayscaleToneType::Mixed;
      } else if (highMidtoneCells >= 13) {
        pixDestroy(&gray);
        return GrayscaleToneType::ContinuousTone;
      }
    }
    pixDestroy(&gray);
    return GrayscaleToneType::BlackWhite;
  }
...
LeptonicaDetector::ColorType detectPrepared(const QImage& image, int midtoneThreshold) {
  ...
  const GrayscaleToneType toneType = grayscale
      ? classifyGrayscaleTone(pix, &midtoneRatio, midtoneThreshold)
      : GrayscaleToneType::ContinuousTone;
  if (!grayscale) {
    result = ColorType::Color;
    fprintf(stderr, "LeptonicaDetector: COLOR (%.1f%% color pixels)\n", colorFraction * 100);
  } else if (toneType == GrayscaleToneType::BlackWhite) {
    result = ColorType::BlackWhite;
    fprintf(stderr, "LeptonicaDetector: B&W (%.1f%% midtones)\n", midtoneRatio * 100);
  } else if (toneType == GrayscaleToneType::Mixed) {
    result = ColorType::Mixed;
...
LeptonicaDetector::ColorType LeptonicaDetector::detect(const QImage& image, int midtoneThreshold) {
  return detectPrepared(image, midtoneThreshold);
}

LeptonicaDetector::ColorType LeptonicaDetector::detectWithCastCompensation(const QImage& image,
                                                                           const int midtoneThreshold) {
  const QImage& analysisImage = image;
  ColorType result = detectPrepared(analysisImage, midtoneThreshold);
  ...
  ColorType recheck = detectPrepared(neutralized, midtoneThreshold);
  if (recheck == ColorType::Color) {
    ...
    if (residualColorFraction < 0.20f) {
      ...
      recheck = detectPrepared(selectivelyNeutralized, midtoneThreshold);
    }
  }
  if (recheck != ColorType::Color) {
