...
class ThumbnailPixmapCache::Impl : public QThread {
 ...
 private:
  ...
  static QImage loadThumbnailSource(const ImageId& imageId, const QSize& maxThumbSize);
  ...
  QThreadPool m_threadPool;  // Parallel thumbnail loading
  Container m_items;
...
ThumbnailPixmapCache::Impl::Impl(const QString& thumbDir,
  ...
  // Configure thread pool for parallel thumbnail loading
...
ThumbnailPixmapCache::Impl::~Impl() {
  ...
  // Wait for all parallel thumbnail loads to complete
...
void ThumbnailPixmapCache::Impl::backgroundProcessing() {
  // This method is called from background thread when there are queued items.
  // We dispatch ALL queued items to the thread pool for parallel loading.

  // Collect items to process while holding the mutex
  struct LoadTask {
...

  {
    ...
    for (LoadQueue::iterator lqIt = m_loadQueue.begin(); lqIt != m_loadQueue.end(); ++lqIt) {
      if (lqIt->status != Item::QUEUED) {
        // All QUEUED items precede any other items in the load queue
        break;
      }

      LoadTask task;
      task.lqIt = lqIt;
      task.imageId = lqIt->imageId;
      task.thumbDir = m_thumbDir;
      task.maxThumbSize = m_maxThumbSize;
      task.expired = (m_totalLoadAttempts - lqIt->precedingLoadAttempts > m_expirationThreshold);
    ...
    // Mark all collected items as IN_PROGRESS
    for (auto& task : tasks) {
      queuedToInProgress(task.lqIt);
    }
  }  // mutex scope

  // Thumbnail generation is subordinate to page processing.  Restrict both
  // concurrency and scheduler priority while a batch occupies the main pool;
  // restore the normal policy for interactive thumbnail-only work.
  if (batch_processing::isActive()) {
    m_threadPool.setMaxThreadCount(2);
    m_threadPool.setThreadPriority(QThread::LowPriority);
  } else {
    m_threadPool.setMaxThreadCount(std::max(1, QThread::idealThreadCount()));
    m_threadPool.setThreadPriority(QThread::InheritPriority);
  }

  // Dispatch all tasks to thread pool for parallel execution
  for (const auto& task : tasks) {
    if (task.expired) {
      postLoadResult(task.lqIt, QImage(), ThumbnailLoadResult::REQUEST_EXPIRED);
      continue;
    }

    // Capture by value for thread safety
    LoadQueue::iterator lqIt = task.lqIt;
    ImageId imageId = task.imageId;
    QString thumbDir = task.thumbDir;
    QSize maxThumbSize = task.maxThumbSize;
    Impl* self = this;

    QtConcurrent::run(&m_threadPool, [self, lqIt, imageId, thumbDir, maxThumbSize]() {
      try {
        ...
        self->postLoadResult(lqIt, image, status);
...
QImage ThumbnailPixmapCache::Impl::loadSaveThumbnail(const ImageId& imageId,
  ...
  image = loadThumbnailSource(imageId, maxThumbSize);
  if (image.isNull()) {
    return QImage();
  }
...
QImage ThumbnailPixmapCache::Impl::loadThumbnailSource(const ImageId& imageId, const QSize& maxThumbSize) {
  const QString& filePath = imageId.filePath();

  // PDF pages are resolution independent, so render them directly at screen
  // resolution instead of paying the project's 300-600 DPI import cost.
  if (PdfReader::canRead(filePath)) {
    // 72 DPI comfortably covers the normal 200 px cache.  Keep unusually
    // high user-selected thumbnail quality useful without returning to source
    // import resolution.
    const int thumbnailRenderDpi = std::max(maxThumbSize.width(), maxThumbSize.height()) > 600 ? 144 : 72;
    return PdfReader::readImage(filePath, imageId.zeroBasedPage(), thumbnailRenderDpi);
  }

  // Qt's JPEG handler uses decoder-side downsampling for setScaledSize().
  // PNG also honors the requested output size and avoids retaining a
  // full-resolution source image in this thumbnail pipeline.
  if (imageId.zeroBasedPage() == 0) {
    QImageReader reader(filePath);
    const QByteArray format = reader.format().toLower();
    if (format == "jpeg" || format == "jpg" || format == "png") {
      const QSize sourceSize = reader.size();
      if (sourceSize.isValid()
          && (sourceSize.width() > maxThumbSize.width() || sourceSize.height() > maxThumbSize.height())) {
        QSize decodedSize(sourceSize);
        decodedSize.scale(maxThumbSize, Qt::KeepAspectRatio);
  ...
  // TIFF and formats without a cheap reduced-resolution decode keep the
  // established full decode + high-quality scale path.
  return ImageLoader::load(imageId);
}
