...
class ThumbnailSequence::Impl {
 ...
 private:
  ...
  void scheduleThumbnailInvalidation(const PageId& pageId);

  void flushPendingThumbnailInvalidations();
  ...
  QRectF m_sceneRect;
  std::set<PageId> m_pendingThumbnailInvalidations;
  bool m_thumbnailInvalidationFlushScheduled = false;
  bool m_selectionLeaderInvalidated = false;
  PageId m_invalidatedSelectionLeaderPageId;
  QSizeF m_oldSelectionLeaderSize;
  QPointF m_oldSelectionLeaderPos;
...
void ThumbnailSequence::Impl::invalidateThumbnail(const PageId& pageId) {
  ...
  if (idIt != m_itemsById.end()) {
    scheduleThumbnailInvalidation(pageId);
  }
}
...
void ThumbnailSequence::Impl::invalidateThumbnail(const PageInfo& pageInfo) {
  ...
  if (idIt != m_itemsById.end()) {
    ...
    scheduleThumbnailInvalidation(pageInfo.id());
  }
}

void ThumbnailSequence::Impl::scheduleThumbnailInvalidation(const PageId& pageId) {
  const ItemsById::iterator idIt(m_itemsById.find(pageId));
  if (idIt != m_itemsById.end() && m_selectionLeader == &*idIt) {
    // Rebuild the visible page now, but leave the strip relayout for the
    // coalesced flush.
    m_pendingThumbnailInvalidations.erase(pageId);
    if (!m_selectionLeaderInvalidated) {
      m_selectionLeaderInvalidated = true;
      m_invalidatedSelectionLeaderPageId = pageId;
      m_oldSelectionLeaderSize = idIt->composite->boundingRect().size();
      m_oldSelectionLeaderPos = idIt->composite->pos();
    }
    invalidateThumbnailImpl(idIt);
  } else {
    m_pendingThumbnailInvalidations.insert(pageId);
  }

  if (!m_thumbnailInvalidationFlushScheduled) {
    m_thumbnailInvalidationFlushScheduled = true;
    QTimer::singleShot(0, &m_owner, [this]() { flushPendingThumbnailInvalidations(); });
  }
}
...
void ThumbnailSequence::Impl::flushPendingThumbnailInvalidations() {
  m_thumbnailInvalidationFlushScheduled = false;
  if (m_pendingThumbnailInvalidations.empty() && !m_selectionLeaderInvalidated) {
    return;
  }

  std::set<PageId> pending;
  pending.swap(m_pendingThumbnailInvalidations);
  for (const PageId& pageId : pending) {
    const ItemsById::iterator idIt(m_itemsById.find(pageId));
    if (idIt != m_itemsById.end()) {
      if (m_selectionLeader == &*idIt && !m_selectionLeaderInvalidated) {
        m_selectionLeaderInvalidated = true;
        m_invalidatedSelectionLeaderPageId = pageId;
        m_oldSelectionLeaderSize = idIt->composite->boundingRect().size();
        m_oldSelectionLeaderPos = idIt->composite->pos();
  ...
  if (m_selectionLeaderInvalidated && m_selectionLeader
      && m_selectionLeader->pageId() == m_invalidatedSelectionLeaderPageId
      && (m_oldSelectionLeaderSize != m_selectionLeader->composite->boundingRect().size()
          || m_oldSelectionLeaderPos != m_selectionLeader->composite->pos())) {
    m_owner.emitNewSelectionLeader(
        m_selectionLeader->pageInfo, m_selectionLeader->composite, REDUNDANT_SELECTION);
  }
  m_selectionLeaderInvalidated = false;
  m_invalidatedSelectionLeaderPageId = PageId();
}

void ThumbnailSequence::Impl::invalidateAllThumbnails() {
  m_pendingThumbnailInvalidations.clear();
  m_thumbnailInvalidationFlushScheduled = false;
  m_selectionLeaderInvalidated = false;
  m_invalidatedSelectionLeaderPageId = PageId();
