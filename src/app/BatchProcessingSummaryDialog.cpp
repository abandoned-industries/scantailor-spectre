...
BatchProcessingSummaryDialog::BatchProcessingSummaryDialog(QWidget* parent)
  ...
  m_timingBreakdownLabel = new QLabel(this);
  m_timingBreakdownLabel->setWordWrap(true);
  QFont breakdownFont = m_timingBreakdownLabel->font();
  breakdownFont.setPointSize(qMax(8, breakdownFont.pointSize() - 1));
  m_timingBreakdownLabel->setFont(breakdownFont);
  m_timingBreakdownLabel->setStyleSheet(QStringLiteral("color: palette(mid);"));
  m_timingBreakdownLabel->setVisible(false);
  mainLayout->addWidget(m_timingBreakdownLabel);
...
void BatchProcessingSummaryDialog::setTimingDetails(const QString& summary, const QString& breakdown) {
  m_timingLabel->setText(summary);
  m_timingLabel->setVisible(!summary.isEmpty());
  m_timingBreakdownLabel->setText(breakdown);
  m_timingBreakdownLabel->setVisible(!breakdown.isEmpty());
}
