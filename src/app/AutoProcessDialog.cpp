...
const char* const kFastOcrKey = "auto_process/fast_ocr";
const char* const kMultilingualOcrKey = "auto_process/multilingual_ocr";
const char* const kSkipOcrLanguageCorrectionKey =
    "auto_process/skip_ocr_language_correction";
}
...
AutoProcessDialog::AutoProcessDialog(QWidget* parent) : QDialog(parent) {
  ...
  fastOcrCheckBox->setChecked(settings.value(kFastOcrKey, true).toBool());
  multilingualOcrCheckBox->setChecked(settings.value(kMultilingualOcrKey, false).toBool());
  skipOcrLanguageCorrectionCheckBox->setChecked(
      settings.value(kSkipOcrLanguageCorrectionKey, true).toBool());
  connect(forceBlackAndWhiteButton, &QRadioButton::toggled,
          this, &AutoProcessDialog::updateRedetectColorEnabled);
  connect(ocrCheckBox, &QCheckBox::toggled,
          this, &AutoProcessDialog::updateOcrControlsEnabled);
  updateRedetectColorEnabled();
  updateOcrControlsEnabled();
...
bool AutoProcessDialog::fastOcr() const {
  return fastOcrCheckBox->isChecked();
}

bool AutoProcessDialog::multilingualOcr() const {
  return multilingualOcrCheckBox->isChecked();
}

bool AutoProcessDialog::skipOcrLanguageCorrection() const {
  return skipOcrLanguageCorrectionCheckBox->isChecked();
}
...
void AutoProcessDialog::accept() {
  ...
  settings.setValue(kFastOcrKey, fastOcr());
  settings.setValue(kMultilingualOcrKey, multilingualOcr());
  settings.setValue(kSkipOcrLanguageCorrectionKey, skipOcrLanguageCorrection());
  QDialog::accept();
}
...
void AutoProcessDialog::updateOcrControlsEnabled() {
  ocrOptionsGroup->setEnabled(includeOcr());
}
